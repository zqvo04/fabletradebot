...
| **v3 연속 포트폴리오 (기본 프로파일)** | **전 게이트 통과** — 전 기간 **+38.21%/MDD −6.0%/샤프 2.16**, 홀드아웃 2026 **+29.01%** (v2 −0.8%), 민감도 8코너 +18~+60%, 비용 2배 +23.9% | [REDESIGN_V3.md](REDESIGN_V3.md) / [VALIDATION_V3.md](VALIDATION_V3.md) |
| **v4 공격 프로파일 (동일 신호, vb 0.4 + 청산 가드)** | **전 게이트 통과** — 전 기간 **+98.3%/MDD −12.9%/샤프 2.41** (월평균 +4.1%), 민감도 8코너 +34~+181%, 비용 2배 +61.4%, MC 95% MDD −20.9%. 확신도 계층 레버리지는 5개 변형 전부 반증·기각 (홀드아웃 다중성 공개 포함) | [REDESIGN_V4.md](REDESIGN_V4.md) / [VALIDATION_V4.md](VALIDATION_V4.md) |
| **v5 상장 인지형 유니버스 + 메이커 집행 (vb 0.2)** | **전 게이트 통과** — 설계 2025 **+50.7%/−10.3%/샤프 2.57** (v4 설계의 2.9배), 전 기간 **+78.2%/−11.8%/샤프 2.23**, 홀드아웃 세그 +18.3%, 비용 2배 +72.7%, MC 95% MDD −22.3%. 위성 20종 룰 편입(연령+유동성 히스테리시스), 확신도 10/5/3/2x는 계좌 하드스톱으로만(`leverage_plan`). vb 0.3 좌석은 수익 정점(+71.9%)이나 MC −30.2%로 게이트 기각 — 생존 편향 공개 포함 | [REDESIGN_V5.md](REDESIGN_V5.md) / [VALIDATION_V5.md](VALIDATION_V5.md) |

**v3 = 아키텍처 교체** (트레이드 선택 → 연속 포트폴리오): 4자산 횡단면 모멘텀
(시장중립 상대강도 롱숏) → 변동성 타게팅 → 노트레이드 밴드 리밸런싱 →
드로다운 거버너. 2025 설계 / 2026 홀드아웃 분리 유지, 홀드아웃이 설계 구간보다
좋음(+29% vs +7% — 과적합의 반대 패턴). 연속 TSM/MR 슬리브는 설계 구간에서
비용을 못 이겨 기각. 상세는 [REDESIGN_V3.md](REDESIGN_V3.md).
(수치는 상장 전 구간까지 포함하는 합집합 인덱스 엔진 기준 — 완성도 평가와
반증 실험 전체 기록은 [ASSESSMENT.md](ASSESSMENT.md).)

**리스크 프론티어** (v3, vol_budget 스케일링): 0.2 → +38%/−6.0%,
...
## 신호 발화 시스템 (v3 — 어떻게 신호가 만들어지고 언제 알림이 오는가)

매 4H봉 마감(00/04/08/12/16/20 UTC) +7분에 워크플로가 실행되어 아래 파이프라인을
결정론적으로 리플레이한다 (시장 데이터 자체가 상태이므로 서버가 죽어도 꼬이지 않음).
v4/v5 프로파일도 동일 파이프라인이며, v5는 [1]의 패널이 "코어 4종 + 룰 편입 위성
(연령·유동성 게이트)"으로 확장되고 집행이 메이커 우선으로 바뀐다 (REDESIGN_V5.md):

```
[1] 신호 계산 (봉 i 종가까지의 정보만 사용 — 룩어헤드 없음)
    자산별 20일(120봉) 수익률을 변동성으로 정규화 → 4자산 횡단면 z-score
    → ±2 클립 후 /2 → XS 신호 ∈ [-1, 1]
    (z > 0: 상대적 강세 = 롱 후보, z < 0: 상대적 약세 = 숏 후보.
     횡단면 평균을 빼므로 시장 방향 베팅이 아니라 자산 간 '순위' 베팅)

[2] 변동성 타게팅 → 목표 비중
    목표비중 = 신호 × (vol_budget 20% / 그 자산의 실현 연변동성)
    변동성이 큰 자산일수록 자동으로 작게 잡힌다. 자산별 레버리지 캡
    (BTC/ETH 1.5x, SOL 1.0x, HYPE 0.5x), 베타 가중 총노출 캡 2.0x 적용.

[3] 드로다운 거버너 (연속 디리스킹 — 전면 정지 없음)
    고점 대비 -6%부터 노출을 선형 축소, -15%에서 25% 플로어.

[4] 집행 필터 (비용 통제)
    목표 비중은 다음 봉 시가에 체결하되, 현재 포지션과의 차이가
    노트레이드 밴드(목표의 50%) 또는 최소 거래(자본의 2%) 미만이면 거래 생략.
    → 회전율을 연 ~50x로 억제 (신호는 매봉, 거래는 필요할 때만)

[5] 신호 발화 → Open 포지션 (Notion 기록 + Telegram)
    자산 목표 비중이 |w| ≥ 0.05로 신규 진입/방향 전환하는 순간(자산당 1개)
    → Notion 신호 로그 DB에 Status=Open 행 생성 (Entry/TP/SL 기록) + Telegram.

[6] 채점 (Win / Loss / Timeout-Win / Timeout-Loss)
    진입가 기준 TP = +2.0σ_day, SL = -1.5σ_day (자산 일간 변동성 배수).
    이후 봉의 고가/저가가 TP 터치 → Win, SL 터치 → Loss (동시 터치는
    보수적으로 SL 우선). 7일 내 미터치 → 타임아웃, 종가 손익 부호로
    Timeout-Win/Timeout-Loss. 판정 시 Notion 행을 Status/Exit/Result R/Closed로
    갱신 + Telegram. 매 실행마다 전체 이력 재계산 후 Notion과 대조(멱등).
```

알림 예시: `🎯 v4 OPEN — SOL 🟢 LONG @ 152.3 · TP 161 / SL 146.1` →
판정 시 `✅ WIN — SOL v4 LONG +1.33R`.

**채점의 의미와 한계 (중요)**: TP/SL 채점은 **매매 로직에 전혀 영향을 주지 않는
진단 오버레이**다 (TP/SL 파라미터는 튜닝하지 않은 측정용 상수). 실측 승률은 41~46%,
평균 R은 0 근처다. **이는 전략이 나쁘다는 뜻이 아니다** — v3/v4의 엣지는 개별 자산이
TP를 때리는 방향성이 아니라 **자산 간 상대강도(시장중립 · 변동성타게팅 · 연속보유)**에서
나오기 때문이다. 채점은 "각 신호가 단기 방향적으로 맞았나"를 보는 별개의 렌즈이며,
포트폴리오 수익성(전 기간 v4 +98%)과 직접 비례하지 않는다. 전체 분포는
`python3 score_history.py`로 재현.

v2 트레이드 엔진(P1 브레이크아웃)은 병행 페이퍼로 돌며 별도 트레이드 일지
(`NOTION_DATABASE_ID`)에 기록된다. 키 설정은 [SECRETS.md](SECRETS.md).
...
fabletradebot/
  ...
  v3.py             # v3/v4/v5 연속 포트폴리오 시스템 (XS 슬리브 + 변동성 타게팅 + 거버너
                    #  + v5: 메이커 집행 모델, 위성 유니버스 게이트, 포트폴리오 vol 캡, 캐리 슬리브)
  leverage_plan.py  # 확신도 10/5/3/2x 계좌 레버리지 티어 (하드스톱/증거금 배분 — 사이징 아님)
  scoring.py        # 신호 채점 오버레이 (Open→TP/SL/타임아웃→Win/Loss, 진단용)
...
run_live_v3.py      # v3/v4/v5 페이퍼/라이브 루프 (V3_PROFILE 선택) + 신호 채점→Notion/Telegram
                    #  (v5는 레버리지 티어 플랜을 state/콘솔에 기록, 라이브 시 set-leverage)
score_history.py    # 전 이력 신호 채점 리포트 (Win/Loss/Timeout 분포)
.github/workflows/paper-trade.yml  # V5 단독 실행 (외부 cron이 workflow_dispatch 호출) + 저널 커밋
CRON_SETUP.md       # cron-job.org로 정시 트리거 연결 가이드 (GitHub schedule 대체)
```
...
- **Notion 일지**: GitHub Secrets에 `NOTION_TOKEN`, `NOTION_DATABASE_ID`(v2 트레이드) /
  `NOTION_SIGNAL_DB_ID`(신호) 설정 시 자동 기록. V5 신호 행에는 **레버리지 티어**
  (2/3/5/10x 하드스톱)와 청산 시 **손익 %**(`PnL %`)가 함께 기록된다 (Telegram 동일).
- **스케줄링**: 운영 워크플로는 V5 프로파일만 돌리며, 정시 실행은 외부 스케줄러
  (cron-job.org)가 `workflow_dispatch`를 호출해 구동한다 — 설정법은 **[CRON_SETUP.md](CRON_SETUP.md)**.
