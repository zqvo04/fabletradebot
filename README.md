...
| **4H + 메이커 청산 (최종 채택)** | **전 게이트 통과** — +1.91%/MDD −4.9%, 민감도 8코너 흑자, 비용 2배 흑자, MC 95% MDD −4.5% | [VALIDATION_4H.md](VALIDATION_4H.md) |

**리스크 스케일링 프론티어** (전 기간 실측): r_base 0.75% → +1.91%/−4.9%,
1.25% → +3.10%/−8.1%, **2.00% → +2.04%/−14.8%** — 리스크를 더 실으면 수익이
오히려 줄어드는 오버켈리 붕괴가 실증됨. 기본값은 0.75%를 유지하고,
페이퍼 트레이딩으로 전방 기대값이 확인된 후에만 1.25%까지 상향을 검토한다.

주의: 홀드아웃(2026 상반기)에서 4H는 −1.4%로 확인에 실패했다(51트레이드,
노이즈 범위). 엣지가 얇고 국면 의존적이라는 뜻이며, 그래서 실탄 전에
페이퍼 전방 검증이 필수다. 게이트는 "지지 않는 시스템"을 보증할 뿐
수익률을 보증하지 않는다.
