...
[5] 신호 발화 (알림/기록 트리거)
    목표 비중이 마지막 발화 시점 대비 |Δ| ≥ 0.03 (자본의 3%p) 움직인 자산만
    "신호 발화"로 판정 → Notion 신호 로그 DB 기록 + Telegram 알림 발송.
    같은 봉에서 재실행돼도 중복 발화하지 않는다 (멱등).
    임계는 SIGNAL_NOTIFY_THRESHOLD 환경변수로 조절.
```

알림 예시: `⚡ v3 신호 — SOL 🟢 LONG / 목표 비중 +0.187 (이전 +0.021, Δ +0.166)`.
LONG/SHORT은 새 목표 비중의 부호, FLAT은 포지션 청산을 뜻한다. v2 트레이드 엔진
(P1 브레이크아웃)은 병행 페이퍼로 돌며 별도 트레이드 일지(`NOTION_DATABASE_ID`)에
기록된다. 키 설정은 [SECRETS.md](SECRETS.md).
