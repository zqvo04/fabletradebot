...
>
> **v0.4 — 템포 최종 결정**: 1H 구제 가설(지정가/메이커 주문 경제학)을
> 2025 설계 구간 / 2026 홀드아웃 분리로 재검증. 1H는 낙관적 비용 상한에서도
> 설계 구간 마이너스로 **최종 기각**, 4H + 메이커 청산이 최종 운영 설정.
> 리스크 프론티어 실측으로 r_base 2%에서 수익 붕괴(오버켈리)를 확인,
> 기본 0.75% 유지. 상세: ANALYSIS_TEMPO.md.
>
> **v0.6 — v3 아키텍처 교체 (최종 운영)**: 트레이드 선택 아키텍처의 한계
> (국면 종속, 저빈도, 오버켈리 절벽)를 넘기 위해 연속 포트폴리오 시스템으로 교체.
> 슬리브 평가에서 연속 TSM/MR은 비용 지배로 기각, 횡단면 모멘텀(시장중립
> 상대강도)만 생존. 전 기간 +42.06%/MDD −6.1%/샤프 2.55, 2026 홀드아웃 +29.05%,
> 게이트 전부 통과. 본 문서의 §1~§5는 v1/v2 트레이드 엔진의 명세이며,
> v3의 설계와 검증은 REDESIGN_V3.md / VALIDATION_V3.md 참조.
>
> **v0.5 — v2 구조 재설계 (트레이드 엔진 최종형)**: v1 검증 트레이드 전수 분해로
> "프로브-피라미드가 승자 사이즈를 깎고, +1R 부분익절이 우측 꼬리를 봉쇄하고,
> P2/P3가 P1의 수익을 잠식"함을 진단. §2.2/§2.3의 프로브-피라미드와 부분익절을
> 폐지(풀 엔트리 + 본전 이동 + 챈들리어 3.25)하고 P1+P4로 집중한
> `config.v2_config()`가 전 게이트 통과 (+8.36%, avgR 8배).
> P1의 CHOP 확장은 홀드아웃에서 §2.3이 예측한 그대로 붕괴하여 기각.
> 상세: REDESIGN_V2.md / VALIDATION_V2.md.
